...
public class AppReactiveUserDetailsService implements ReactiveUserDetailsService {
    ...
    public Mono<UserDetails> findByUsername(String username) {
        ...
        Mono<UserDetails> userDetailsMono = databaseClient.sql(
                                                                  "select u.*,r.id rid,r.name,r.value,pm.id pid,pm" +
                                                                          ".value pvalue,pm.description " +
                                                                          "from t_user u " +
                                                                          "left join t_user_role ur on ur.user_id=u" +
                                                                          ".id " +
                                                                          "left join t_roles r on r.id = ur.role_id " +
                                                                          "left join t_role_perm rp on rp.role_id=r" +
                                                                          ".id " +
